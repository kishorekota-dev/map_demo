...
router.get('/', 
  createResourceSecurityMiddleware('balanceTransfers', 'read'),
  ...
  async (req, res) => {
    try {
      ...
      // Get balance transfers based on role access
      let transfers = SecurityUtils.filterByUserAccess(mockData.balanceTransfers, userRole, userId);
      ...
      const paginatedTransfers = transfers.slice(offset, offset + actualLimit);

      // Format response with sanitized data
      const formattedTransfers = paginatedTransfers.map(transfer => {
        const sanitized = SecurityUtils.sanitizeDataByRole(transfer, userRole, ['internalNotes']);
        return {
          ...sanitized,
          formattedAmount: formatCurrency(sanitized.amount),
          formattedFee: formatCurrency(sanitized.fee),
          formattedTotalAmount: formatCurrency(sanitized.amount + sanitized.fee)
        };
      });

      // Log access
      await SecurityUtils.logSecurityEvent(req, 'BALANCE_TRANSFER_LIST_ACCESS', {
        totalTransfers: formattedTransfers.length,
        filters: { status, transferType, startDate, endDate }
      });
